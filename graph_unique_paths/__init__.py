...
class Graph:
    def _get_nodes(self, graph, boolean_condition):
        nodes = []
    ...
    def solve(self, json_file_path):
        ...
        """
        graph = self._get_json_string(json_file_path)
        start_nodes = self._get_nodes(graph, 'is_start_node')
        end_nodes = self._get_nodes(graph, 'is_end_node')
        all_paths = []

        for end_node in end_nodes:
            for start_node in start_nodes:
                all_paths.append(self._find_all_paths(graph, start_node, end_node))
